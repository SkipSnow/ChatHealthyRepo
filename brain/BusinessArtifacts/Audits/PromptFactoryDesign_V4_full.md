...
The architecture uses the aircraft assembly pattern. There are five participants:

The PromptAssemblyFactory is the final assembly line. It knows the blueprint — where each part mounts, what forces each part creates, what order assembly proceeds, and how to wire the connections between sub-factories. It does NOT understand the content of any part. It is deliberately dumb.
...
1. Slot layout — the order of slots: system_context, then constraints, then user_intent, then tools, then LLM call, then post_processing.

2. Sub-factory registry — which sub-factories exist and their capability declarations (their APIs). Not their internals.

3. Routing rules — given user input plus session state, which epic sub-factory handles this request. This is the assembly factory's one non-trivial responsibility. Routing rules are explicit and declarative. Each epic sub-factory declares what intents it handles. The assembly factory matches user intent to declarations. Ambiguous intents default to FindCare (the primary epic).

4. Dependency resolution — if a PromptPart says it depends_on safety_status, the assembly factory ensures the safety check runs first. If it depends_on llm_response, the assembly factory ensures the LLM call happens before that part runs.

5. Force wiring — if any part says force=HALT, the assembly stops. If force=DOWNGRADE, the assembly swaps the LLM model. If force=OVERLAY, the assembly wraps the response with the overlay content.

6. Session state management — the assembly factory maintains session state (last_state, last_specialty, off_topic_count, consent_status, message_history) and passes it to sub-factories that require it.
...
If we give it more than the six things above, it becomes a god object. If we give it less, it cannot assemble correctly.

### 3.6 The Routing Decision

The routing decision is the assembly factory's one non-trivial responsibility. It must classify user intent to pick the right epic sub-factory. This is the area most at risk of becoming god-like.
...
Domain: Provider search, specialty resolution, geographic context carry-forward. The core FindCare epic. This is the primary epic — ambiguous intents default here.
...
Domain: Provider quality evaluation, clinical trials search, provider detail lookup. The Evaluate Care epic. This sub-factory handles requests that go beyond finding a provider — evaluating provider quality, searching for clinical trials, and looking up detailed provider information.
...
2. Sub-factory registry — which sub-factories exist, and their capability declarations (their supply/require/force APIs). Not their internals.

3. Routing rules — given user input plus session state, which epic sub-factory handles this request. Each epic sub-factory declares what intents it handles. The assembly factory matches user intent to those declarations. Ambiguous intents default to FindCare.

4. Dependency resolution — SharedServicesFactory.safety_status must resolve before any epic sub-factory runs. Post-processing parts depend on the LLM response existing.

5. Force wiring — HALT from SharedServicesFactory means skip all epic sub-factories and return the emergency response. DOWNGRADE means swap the LLM model to a commodity model. OVERLAY means wrap the final response with a consent dialog.
...
Internal class diagram plus narrative explaining the assembly flow, session state management, routing logic, and force propagation.

### 6.2 Package: SharedServicesFactory

Internal class diagram plus narrative explaining safety detection, consent workflow, URL validation, CPT redaction, off-topic detection, and lead capture.

### 6.3 Package: FindCarePromptFactory

Internal class diagram plus narrative explaining provider search, specialty vector search, context carry-forward, and filter options.

### 6.4 Package: NarrativePromptFactory

Internal class diagram plus narrative explaining conversational responses and about content.

### 6.5 Package: EvaluateCarePromptFactory
...
### 7.3 Clinical Trials Search
### 7.4 Emergency/Safety Detection (SharedServicesFactory HALT force)
### 7.5 Context Carry-Forward — Virginia then Richmond
### 7.6 Consent Trigger (SharedServicesFactory OVERLAY force, conditional)
### 7.7 URL Validation on Tool Response (SharedServicesFactory post_processing)
