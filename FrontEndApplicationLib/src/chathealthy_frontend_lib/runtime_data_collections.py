"""Runtime data collection bindings driven by Config.DBVersions.
...
`bind_from_manifest()` at FastAPI startup. The function:
  ...
  2. Reads Config.DBVersions.find_one({"env": env}) on the
...
def _read_env_doc(env: str) -> dict:
    ...
    if doc is None:
        raise RuntimeError(
            f"runtime_data_collections: Config.DBVersions has no document "
...
def _bindings_from_doc(doc: dict, target_id: str) -> dict[str, str]:
    ...
    raise RuntimeError(
        f"runtime_data_collections: Config.DBVersions env={doc.get('env')!r} "
...
def bind_from_manifest() -> None:
    """Read build_info.json + Config.DBVersions and bind module-level
...
def debug_active_collections(
    ...
    env's Config.DBVersions document. state is 'stable' when they match,
