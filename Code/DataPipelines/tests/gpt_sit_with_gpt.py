...
def _build_initial_prompt(requirements: list) -> str:
    parts = [
        "# SIT ASSIGNMENT: GPT Reader Service",
        "",
        f"Test all 35 requirements. You have {MAX_ITERATIONS} iterations.",
        "Send one JSON command per iteration. I execute it and return the result.",
        "If you find a bug, log it and KEEP TESTING. Do not stop on bugs.",
        "Do NOT submit your report until you have attempted every testable requirement. Early submission is a testing failure.",
        "",
        "## REQUIREMENTS",
        "",
    ]
    for r in requirements:
        tag = f" [{r['status'].upper()}]" if r['status'] == 'deferred' else ""
        parts.append(f"- **{r['id']}**: {r['requirement']}{tag}")
    parts.extend([
        "",
        ...
        '  "report_title": "GPT SIT Report — GPT Reader Service",',
        '  "design_version": "6.2",',
        '  "overall_verdict": "PASS or FAIL",',
        '  "summary": {"total_requirements": 35, "pass": 0, "fail": 0, "untested": 0},',
        '  "results": [',
        '    {"requirement_id": "R1", "verdict": "PASS|FAIL|UNTESTED", "test_evidence": "exact command and response", "notes": ""},',
        '    {"requirement_id": "R2", "verdict": "...", "test_evidence": "...", "notes": ""},',
        '    ... one entry for EVERY requirement R1 through R35 ...',
        '  ],',
        '  "bugs": [',
        ...
        '  ],',
        '  "sign_off": "I tested X of 35 requirements. Y passed, Z failed, W untested."',
        '}}',
        "",
        "Verdicts: PASS = tested and working. FAIL = tested and broken. UNTESTED = you did not test it.",
        "Every requirement R1-R35 MUST appear in results. No exceptions.",
        "",
