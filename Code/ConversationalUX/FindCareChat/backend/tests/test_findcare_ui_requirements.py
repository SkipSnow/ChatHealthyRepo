...
EPIC-006-F-001-S-002-REQ-B-002: Drag and drop provider from top to bottom list
EPIC-006-F-001-S-002-REQ-B-007: Only selected providers (bottom list) sent to EvaluateCare
FINDCARE-UX-002: Specialty list >15 items displays in scroll window
EPIC-006-F-002-S-001-REQ-B-010: Toggle label reads 'Uncheck All' / 'Check All' based on majority
EPIC-006-F-021-S-001-REQ-B-004: Control frame is 7% of center column height
...
# ── EPIC-006-F-001-S-002-REQ-B-002 ─────────────────────────────────────────────────
# User can drag and drop provider from top to bottom list

def test_drag_drop_code_infrastructure():
    """EPIC-006-F-001-S-002-REQ-B-002: FindCareApp has drag-and-drop handlers for provider selection."""
...
def test_drag_drop_live_dom():
    """EPIC-006-F-001-S-002-REQ-B-002: live DOM has the drop target div."""
...
# ── EPIC-006-F-001-S-002-REQ-B-007 ─────────────────────────────────────────────────
# Only selected providers (bottom list) sent to EvaluateCare

def test_only_selected_sent_to_evaluate_care():
    """EPIC-006-F-001-S-002-REQ-B-007: handleEvaluate sends only selectedRef.current to EvaluateCare."""
    src = _read("FindCareApp.tsx")
...
def test_toggle_all_button_in_findcare_app():
    """EPIC-006-F-002-S-001-REQ-B-010: FindCareApp has toggle-all button with 'Uncheck All' label."""
    src = _read("FindCareApp.tsx")
    assert 'data-gui-action="toggle-all"' in src, "Must have toggle-all button"
    assert "Uncheck All" in src, "Default label must be 'Uncheck All'"


# ── EPIC-006-F-021-S-001-REQ-B-004 ─────────────────────────────────────────────────
# Control frame is 7% of center column height

def test_control_frame_7_percent_in_html():
    """EPIC-006-F-021-S-001-REQ-B-004: index.html has guiControlFrame with height:7%."""
...
def test_control_frame_7_percent_live():
    """EPIC-006-F-021-S-001-REQ-B-004: live page has guiControlFrame element with height."""
    from playwright.sync_api import sync_playwright
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_context(viewport={"width": 1400, "height": 900},
