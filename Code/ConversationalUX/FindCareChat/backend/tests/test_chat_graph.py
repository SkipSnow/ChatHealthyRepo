...
def _make_graph(
    ...
    graph = build_graph(
        ...
        url_guardian=MagicMock(guard_text=lambda t: t),
...
class TestNormalPath:
    ...
    def test_returns_llm_response(self):
        graph, llm_mock, _, _ = _make_graph()
    ...
    def test_returns_token_counts(self):
        graph, _, _, _ = _make_graph(llm_tokens_in=200, llm_tokens_out=75)
    ...
    def test_search_term_extracted(self):
        graph, _, _, _ = _make_graph(search_term="shrinks")
...
class TestEmergencyPath:
    ...
    def test_returns_emergency_response(self):
        graph, _, _, _ = _make_graph(safety_emergency=True)
...
class TestIPLockedPath:
    ...
    def test_returns_emergency_response(self):
        graph, _, _, _ = _make_graph(safety_ip_locked=True)
        result = graph.invoke({"user_message": "hello", "history": [], "ip": "1.2.3.4"})
        assert result["final_response"]["response"] == "CALL 911"

    def test_llm_not_called(self):
        graph, llm_mock, _, _ = _make_graph(safety_ip_locked=True)
