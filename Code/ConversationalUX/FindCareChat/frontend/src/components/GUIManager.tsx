...
function renderEvaluateButtonHTML(): string {
  return `
    <button data-gui-action="evaluate-providers"
      style="padding: 1em;border-radius:0.625em;font-size: 1em;font-weight:600;
        font-family:system-ui,sans-serif;cursor:pointer;color:#fff;
        background:linear-gradient(180deg,#0b9a94,#0b7a75);border:none;
        border-bottom:0.375em solid #065a56;
        box-shadow:0 0.25em 0.5em rgba(0,0,0,0.25),inset 0 0.125em 0 rgba(255,255,255,0.15);
        transition:all 0.08s ease;user-select:none;margin-left: 1em;"
      onmousedown="this.style.transform='translateY(0.25em)';this.style.boxShadow='inset 0 0.25em 0.5em rgba(0,0,0,0.2)'"
      onmouseup="this.style.transform='';this.style.boxShadow='0 0.25em 0.5em rgba(0,0,0,0.25),inset 0 0.125em 0 rgba(255,255,255,0.15)'"
      onmouseleave="this.style.transform='';this.style.boxShadow='0 0.25em 0.5em rgba(0,0,0,0.25),inset 0 0.125em 0 rgba(255,255,255,0.15)'"
...
function renderPaginationHTML(state: PaginationState): string {
  ...
  const btn3d = (enabled: boolean) => `
    ...
    border:none;
    border-bottom:${enabled ? `0.375em solid #4b5563` : `0.25em solid #c0c0c0`};
    box-shadow:${enabled ? '0 0.25em 0.5em rgba(0,0,0,0.25), inset 0 0.125em 0 rgba(255,255,255,0.15)' : 'none'};
    transition:all 0.08s ease;
    user-select:none;
  `.replace(/\n\s+/g, '')

  // Mouse down: warm gray, depressed
  const pressDown = `this.style.background='linear-gradient(180deg, ${warmGray} 0%, #57534e 100%)';this.style.borderBottom='0.125em solid #44403c';this.style.transform='translateY(0.25em)';this.style.boxShadow='inset 0 0.25em 0.5em rgba(0,0,0,0.2)';`
  // Mouse up: back to cold gray, raised
  const pressUp = `this.style.background='linear-gradient(180deg, #8b8f96 0%, ${coldGray} 100%)';this.style.borderBottom='0.375em solid #4b5563';this.style.transform='translateY(0)';this.style.boxShadow='0 0.25em 0.5em rgba(0,0,0,0.25), inset 0 0.125em 0 rgba(255,255,255,0.15)';`

  return `
    <div style="display:flex;align-items:center;justify-content:center;gap:2.5em;padding: 1em;font-size: 1em;font-family:system-ui,sans-serif;height:100%;">
      ...
      >&laquo; Back</button>
      <span style="color:#374151;font-weight:600;font-size: 1em;min-width:20em;text-align:center;letter-spacing:0.02em;">
...
function renderFilterHTML(options: any[], totalProviders: number = 0, prescribersChecked: boolean = true, homeopathicChecked: boolean = false): string {
  ...
  const items = options.map(opt =>
    `<div style="display:block;padding: 1em;border-bottom:0.125em solid #eee;" data-spec-code="${opt.code}" data-can-prescribe="${opt.can_prescribe || false}" data-homeopathic="${opt.homeopathic || false}">
      <label style="display:flex;align-items:center;gap:1em;cursor:pointer;font-size: 1em;line-height:1.3;">
        <input type="checkbox" data-gui-action="filter-toggle" data-gui-value="${opt.code}" checked
          style="accent-color:#0b7a75;width:1.75em;height:1.75em;flex-shrink:0;" />
  ...
  return `
    <div data-filter-panel style="display:flex;flex-direction:column;font-family:system-ui,sans-serif;background:#fff;">
      <div style="padding: 1em;border-bottom:0.25em solid #0b7a75;background:#f8fffe;">
        <div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;">
          <div style="flex:0 0 auto;padding-right: 1em;border-right:0.125em solid #d8e2e1;">
            <div style="font-size: 1em;font-weight:700;color:#0b7a75;text-transform:uppercase;letter-spacing:0.03em;white-space:nowrap;">Filter by specialty</div>
          </div>
          <div style="flex:0 0 auto;padding: 1em;border-right:0.125em solid #d8e2e1;text-align:center;">
            <div style="font-size: 1em;color:#6b7280;text-transform:uppercase;">All possible</div>
            <div style="font-size: 1em;font-weight:700;color:#1f2937;" id="filterAllCount">${allCount}</div>
          </div>
          <div style="flex:0 0 auto;padding: 1em;border-right:0.125em solid #d8e2e1;text-align:center;">
            <div style="font-size: 1em;color:#6b7280;text-transform:uppercase;">${presHomeoLabel}</div>
            <div style="font-size: 1em;font-weight:700;color:#1f2937;" id="filterFilteredCount">${filteredCount}</div>
          </div>
          <div style="flex:0 0 auto;padding: 1em;border-right:0.125em solid #d8e2e1;text-align:center;">
            <div style="font-size: 1em;color:#6b7280;text-transform:uppercase;">Your choices</div>
            <div style="font-size: 1em;font-weight:700;color:#0b7a75;" id="filterShowing">${checkedCount}</div>
          </div>
          <div style="flex:0 0 auto;padding-left: 1em;display:flex;flex-direction:column;gap:0.25em;justify-content:center;">
            <label style="font-size: 1em;color:#1f2937;display:flex;align-items:center;gap:0.375em;cursor:pointer;white-space:nowrap;">
      ...
      <div style="padding: 1em;display:flex;align-items:center;border-bottom:0.125em solid #e5e7eb;background:#fafafa;">
        <button data-gui-action="toggle-all"
          style="background:none;border:0.125em solid #0b7a75;border-radius:0.375em;padding: 1em;
          font-size: 1em;color:#0b7a75;cursor:pointer;font-weight:600;">Uncheck All</button>
      </div>
      <div style="max-height:48.75em;overflow-y:auto;overflow-x:hidden;">
        ${items}
      </div>
      <div style="padding: 1em;border-top:0.125em solid #d8e2e1;">
        <button data-gui-action="filter-apply" style="width:100%;padding: 1em;border-radius:0.5em;
          border:none;background:linear-gradient(180deg,#0b9a94,#0b7a75);color:#fff;
          font-size: 1em;font-weight:600;cursor:pointer;
          border-bottom:0.25em solid #065a56;box-shadow:0 0.125em 0.375em rgba(0,0,0,0.2);"
          onmousedown="this.style.transform='translateY(0.125em)';this.style.boxShadow='none'"
          onmouseup="this.style.transform='';this.style.boxShadow='0 0.125em 0.375em rgba(0,0,0,0.2)'"
          onmouseleave="this.style.transform='';this.style.boxShadow='0 0.125em 0.375em rgba(0,0,0,0.2)'"
        >Apply Filter</button>
        <button data-gui-action="evaluate-providers" style="width:100%;padding: 1em;border-radius:0.5em;
          border:none;background:linear-gradient(180deg,#d97706,#b45309);color:#fff;
          font-size: 1em;font-weight:600;cursor:pointer;margin-top: 1em;
          border-bottom:0.25em solid #92400e;box-shadow:0 0.125em 0.375em rgba(0,0,0,0.2);"
          onmousedown="this.style.transform='translateY(0.125em)';this.style.boxShadow='none'"
          onmouseup="this.style.transform='';this.style.boxShadow='0 0.125em 0.375em rgba(0,0,0,0.2)'"
          onmouseleave="this.style.transform='';this.style.boxShadow='0 0.125em 0.375em rgba(0,0,0,0.2)'"
        >Evaluate These Providers</button>
