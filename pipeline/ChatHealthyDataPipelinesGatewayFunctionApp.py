...
def _map_provider_pipeline_args(args: dict) -> dict:
    """LLD v22 upstream mapping with backward-compatible start_step."""
    out = dict(args)
    if out.get("start_step") and not out.get("resume_from_step"):
        out["resume_from_step"] = out["start_step"]
    if out.get("job_id") and not out.get("run_id"):
        out["run_id"] = out["job_id"]
    return out
...
def gateway_router(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        if task in _AUTOMATION_ROUTES:
            ...
            payload_args = _map_provider_pipeline_args(args) if task == "ProviderPipeline" else args
            upstream_body = {
                "ChatHealthyTask": task,
                "payload": {
                    **payload_args,
