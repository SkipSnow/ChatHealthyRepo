...
def test_no_raw_requests_get():
    """no requests.get in prescriber_evaluate_care_pipeline.py"""
    content = _read("prescriber_evaluate_care_pipeline.py")
    assert "requests.get(" not in content

def test_no_readall():
    """no .readall() on full blob downloads in pipeline files.
...
def test_no_hardcoded_urls():
    """CMS URL loaded from env/config, not used as bare literal"""
    content = _read("prescriber_evaluate_care_pipeline.py")
