"""POST the Function App's Router for ProviderPipeline across the 10 largest
US states by provider population and assert 202.

States: CA, TX, FL, NY, PA, IL, OH, GA, NC, NJ.

Repeatable invocation of the exact same HTTP request a hand-run of
`pipeline_trigger.py ProviderPipeline --payload-json
'{"states":["CA","TX","FL","NY","PA","IL","OH","GA","NC","NJ"],"pipeline_cluster":"ChatHealthyDataPipelines"}'`
produces.
...
def test_provider_pipeline_large_multi_state_returns_202() -> None:
    token = _bearer_token()
    resp = requests.post(
        ROUTER_URL,
        json=BODY,
