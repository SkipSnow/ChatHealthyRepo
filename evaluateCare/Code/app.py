...
#
# The transport shell — the app object, the CORS policy, and the two
# exception handlers — is the generic Gate in chathealthy_lib.gate; it is
# the one node that knows transport exists. EvaluateCare is a satellite: it
# carries no navigator, so it mounts no /gate. Each remaining FastAPI route
# constructs its dedicated endpoint class and runs it. Endpoint classes live
# in displayChrome/, healthcheck/, externalInterface/.
...
# the generic Gate; EvaluateCare names its own component and origins.
app = ch_gate.build_gate_app(
    title="ChatHealthy.ai EvaluateCare",
    ...
    cors_allow_origin_regex=r"https://localhost(:\d+)?$|https://[a-zA-Z0-9-]+\.chathealthy\.ai$|https://skipsnow-[a-zA-Z0-9-]+\.hf\.space$",
