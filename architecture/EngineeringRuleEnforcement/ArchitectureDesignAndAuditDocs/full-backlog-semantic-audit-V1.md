...
### High (4)
- **B7-001** — 4 unresolved EPIC-008 req IDs in 3 git-tracked files outside the consolidation pass.
- **F2-001** — Consolidation Word report stale (anchors to V18; counts off by +4).
- **F3-001** — `scan_files_enforcement_worker.py:439` claims V18 is "the binding contract".
- **G1-001** — 2 high-severity Orphan bugs (BUG-SCANNER-OVERSCOPE-001, BUG-AGILE-BACKLOG-UNIQUENESS-DISABLED-001) with no req_id.
- **G2-001** — BUG-ENF-WORKER-001 and BUG-ENF-WORKER-002 still `in_analysis` despite being logically resolved.

### Medium (7)
- **B1-001** — 3 bugs cite strict-format req_ids that don't resolve.
- **B3-001** — 57 of 65 depends_on entries don't resolve.
- **D3-002** — TR-1, TR-2, TR-9, TR-10, TR-13 backlog wording is materially shorter than V19 (loses binding details).
- **E1-001** — `chathealthy_enforcement_manager.py` internal docstrings cite V17 throughout.
- **E2-001** — `enforcement_worker.py` internal docstrings cite V17 throughout.
- **E3-001** — `scan_files_enforcement_worker.py` internal docstrings cite V18 throughout.
- **E5-001** — TR-13 is forward-looking and not yet implemented (preCommitScan.py still contains the legacy patterns); needs status tracking.
- **F1-001** — Test fixture `04_missing_local_schema.json` has a misleading legacy name.
- **H1-001** — Same as F2-001 from a different angle.

### Low (9)
- **A7-001** (Informational) — 46 orphan reqs lacking req_id.
- **B1-003** — 8 bugs use non-standard non-EPIC req_id formats.
- **D3-001** — Minor BR paraphrase drift (BR-3, BR-8, BR-11, BR-14).
- **D4-001** — F-002 story descriptions reference V18 instead of V19.
- **E4-001** — Test files don't tag which TR each test exercises.
- **F3-002** — Other V17/V18 stale labels in framework docstrings.

### Informational (4)
- **B1-002** — 8 bugs use sentinel orphan markers.
- **B2-001** — 89% of pytest entries are orphan.
- **B4-001** — realized_by is intentionally a free-text array of tech labels, not a FK.
- **I1-001** — Defensive code is fully TR-traced.
