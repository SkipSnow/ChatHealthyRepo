...
def _project_root() -> Path:
    """The repository root, found by its marker rather than by counting.

    This used to be parents[4], correct only while the file sat four levels
    down in Code/Shared/ops/tools. Moving it to the feature that owns it --
    three levels down -- made parents[4] resolve to the drive root, and the
    boot died looking for C:/brain/machine_artifacts/content. A depth count
    is a claim about where a file lives, and the refactor's whole purpose is
    to move files.
    """
