...
class CommitGovernanceDriver:
    def __init__(self, entire_tree: bool = False) -> None:
    ...
    def _empty_the_stash(self) -> int:
        """Apply every stash, so nothing hides from a whole-tree scan.

        A stashed change is a change the promote would otherwise carry
        forward invisibly -- present on the operator's disk, absent from the
        baseline, and never examined.
        """
        applied = 0
        while self._git("stash", "list").strip():
            self._git("stash", "pop")
            applied += 1
        return applied
    ...
    def collect(self) -> list[str]:
        ...
        if self.entire_tree:
            popped = self._empty_the_stash()
            if popped:
                _CH_LOG.info(f"[driver] applied {popped} stash(es)")
            self._git("add", "-A")
    ...
    def _record(self, results: list[dict[str, Any]], verdict: int) -> dict[str, Any]:
        ...
        for r in results:
            for v in r["violations"]:
                ...
                failed_by_file.setdefault(resource, []).append(r["enforcement_id"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        return CommitGovernanceDriver(entire_tree=args.entire_tree).run()
