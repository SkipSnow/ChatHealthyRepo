...
class SessionEndpoint:
    """GET /session — mint a SharedServices-signed session token.
    ...
    def __init__(self):
        self.log = logging.getLogger("shared_services.session")
        self.env_prefix = os.getenv("ENV_PREFIX", "dev")

    def __call__(self) -> SessionToken:
        raw = generate_session_token("SharedServices")
        ...
        self.log.info(
            "Minted SharedServices session token: env=%s nonce_len=%d",
